//@@viewOn:imports
import UU5 from "uu5g04";
import "uu5g04-bricks";
import { createVisualComponent, useEffect } from "uu5g04-hooks";
import Plus4U5 from "uu_plus4u5g01";
import "uu_plus4u5g01-app";

import Config from "./config/config.js";
import Lsi from "../config/lsi.js";
//@@viewOff:imports

const STATICS = {
  //@@viewOn:static
  displayName: Config.TAG + "Left",
  //@@viewOff:static
};

export const Left = createVisualComponent({
  ...STATICS,

  //@@viewOn:propTypes
  //@@viewOff:propTypes

  //@@viewOn:defaultProps
  //@@viewOff:defaultProps

  render(props) {
    //@@viewOn:private
    const items = [
      {
        id: "home",
        href: "home",
        content: <UU5.Bricks.Lsi lsi={Lsi.left.home} />,
        onClick: () => props.setInitialActiveItemId("home"),
      },
      {
        id: "About",
        href: "About",
        content: <UU5.Bricks.Lsi lsi={Lsi.left.About} />,
        onClick: () => props.setInitialActiveItemId("About"),
      },
      {
        id: "CatList",
        href: "CatList",
        content: <UU5.Bricks.Lsi lsi={Lsi.left.CatList} />,
        onClick: () => props.setInitialActiveItemId("CatList"),
      },
      {
        id: "CatDetails",
        href: "CatDetails",
        content: <UU5.Bricks.Lsi lsi={Lsi.left.CatDetails} />,
        onClick: () => props.setInitialActiveItemId("CatDetails"),
      },
    ];
    //@@viewOff:private

    const { useCase } = UU5.Common.Url.parse(window.location.href);
    useEffect(() => {
      if (useCase !== "about") {
        const find = items.find((item) => item.href === useCase);
        props.setActiveItemId(find.id);
      }
    }, []);
    //@@viewOn:interface
    //@@viewOff:interface

    //@@viewOn:render
    return (
      <Plus4U5.App.Left
        {...props}
        logoProps={{
          backgroundColor: UU5.Environment.colors.blue.c700,
          backgroundColorTo: UU5.Environment.colors.blue.c500,
          title: "uuJokes",
          companyLogo: Plus4U5.Environment.basePath + "assets/img/unicorn-logo.svg",
          generation: "1",
        }}
        aboutItems={[{ content: <UU5.Bricks.Lsi lsi={Lsi.left.about} />, href: "about", id: "aboutItems" }]}
        helpHref={null}
      >
        <Plus4U5.App.MenuTree
          borderBottom
          // NOTE Item "id" equals to useCase so that item gets automatically selected when route changes (see spa-autheticated.js).
          items={items}
        />
      </Plus4U5.App.Left>
    );
    //@@viewOff:render
  },
});

export default Left;
